    ...
    pub fn evaluate(&self, p: &Process) -> Result<bool, String> {
        match self {
            ...
            Clause::Expr(field, op, value) => match (field, op, value) {
                ...
                (Field::Pid, op, Value::Unitless(value)) => {
                    Ok(op.compare_numeric(p.stat.pid, *value))
                }
                (Field::Uid, op, Value::Unitless(uid)) => {
                    Ok(op.compare_numeric(p.owner, *uid as u32))
                }
                (Field::Rss, op, Value::Bytes(bytes)) => {
                    Ok(op.compare_numeric(p.stat.rss_bytes(), *bytes as i64))
                }
                (Field::Rss, op, Value::Percent(pct)) => Ok(op.compare_numeric(
                    p.stat.rss_bytes(),
                    (MEMINFO.mem_total as f64 * *pct as f64) as i64,
                )),
                (Field::Vsize, op, Value::Bytes(bytes)) => {
                    Ok(op.compare_numeric(p.stat.vsize, *bytes as u64))
                }
                (Field::Vsize, op, Value::Percent(pct)) => Ok(op.compare_numeric(
                    p.stat.vsize,
...
impl Field {
    ...
    fn build_value_from_str(&self, s: &str) -> Result<Value, String> {
        match *self {
            ...
            Field::Pid => i32::from_str_radix(s, 10)
                .map(Value::Unitless)
                .map_err(|e| format!("Unable to parse as int: {:?}", e)),
            Field::Uid => {
                // if the string is parsable as an int, use that.  else try look up the provided string as a username
                i32::from_str_radix(s, 10)
                    .map(Value::Unitless)
                    .or_else(|_e| util::lookup_uid(s).map(|i| Value::Unitless(i as i32)))
            }
            Field::Rss | Field::Vsize => {
                if s.ends_with('%') {
                    use std::str::FromStr;
                    let p = f32::from_str(&s[..s.len() - 1])
                        .map_err(|_| format!("Unable to parse {} as a percentage", s))?;
                    if p < 0.0 {
                    ...
                    BYTES_REGEX
                        ...
                        .and_then(|caps| {
                            Some((
                                caps.get(1)?.as_str(),
                                caps.get(2).map(|x| x.as_str()).unwrap_or("B"),
                            ))
                        })
                ...
                .and_then(|(num_part, unit_part)| {
                    ...
                    match unit_part {
                        "s" | "sec" | "second" | "seconds" => {
                            Some(Value::Duration(Duration::seconds(num_part)))
                        }
                        "m" | "min" | "minute" | "minutes" => {
                            Some(Value::Duration(Duration::minutes(num_part)))
                        }
                        "h" | "hr" | "hour" | "hours" => {
                            Some(Value::Duration(Duration::hours(num_part)))
                        }
            ...
            Field::State => {
                if s.len() == 1 {
                    ProcState::from_char(s.chars().next().unwrap())
                        .map(Value::State)
                        .ok_or(format!("Unable to parse {:?} as a process state", s))
                } else {
                    match s {
